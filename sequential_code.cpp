...
int main(int argc, char *argv[])
{
    ...
    for (int frame = 0; frame < RENDERED_FRAMES; frame++)
    {
        ...
        simulationStepNoParallel(shapes, dt, totalCollisions);
        ...
        encoder.writeFrameRGB(current_rgb);
        auto end_write = std::chrono::high_resolution_clock::now();
        double time_write = std::chrono::duration<double, std::milli>(end_write - start_write).count();

        writing_duration += time_write;
    }
    ...
    if (DEBUG_PERF) {
        ...
        std::cout << "Average Writing Time:    " << writing_duration / duration_total << " ms\n";
